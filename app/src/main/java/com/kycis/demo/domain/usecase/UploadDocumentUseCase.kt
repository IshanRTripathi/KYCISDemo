...
import com.kycis.demo.domain.models.DocumentType
...
class UploadDocumentUseCase @Inject constructor(
    ...
    suspend operator fun invoke(document: ImageData, type: DocumentType): Result<UploadResponse> {
        // Validate image first
        val validationResult = imageProcessor.validateImage(document)
        ...
        // Compress if needed
        val processedImage = imageProcessor.compressImage(document, 2048) // 2MB

        // Upload through repository
        return kycRepository.uploadDocument(processedImage, type)
    }
}